...
<section id="content" class="flex">
    ...
    <div id="menu-content" class="flex flex-col w-full pb-[30px]">
        ...
        @if($errors->any())
            @foreach($errors->all())
                <div class="py-5 px-5 bg-red-700 text-white">
                    {{$error}}
                </div>
            @endforeach

        @endif
        <form method="post" action="{{ route('dashboard.course.update', $course) }}" enctype="multipart/form-data"
              class="flex flex-col gap-[30px] w-[500px] mx-[70px] mt-10">
            @csrf
            @method('PUT')
            <div class="flex gap-5 items-center">
                <input type="file" name="icon" id="icon" class="peer hidden" onchange="previewFile()"
                       data-empty="true"
                >
                <div
            ...
            <div class="group/category flex flex-col gap-[10px]">
                ...
                <div
                    ...
                    <select id="category"
                        ...
                        <option value="{{$course->category->id}}" class="font-semibold" hidden disabled
                                selected>{{$category->name}}</option>
                        @forelse($categories as $category)
                            <option value="{{$category->id}}" class="font-semibold">{{$course->category->name}}</option>
                        @endforelse
                    </select>
                </div>
            </div>
